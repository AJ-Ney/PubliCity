...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_noticias_modificar`(IN `_id` INT, IN `_categoria` INT, IN `_titulo` VARCHAR(150), IN `_image` MEDIUMBLOB, IN `_subtitulo` VARCHAR(100), IN `_descripcion` TEXT, IN `_redactor` VARCHAR(50))
BEGIN
UPDATE tb_noticias SET
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_noticias` AS select `n`.`id` AS `id`,`c`.`categoria` AS `categoria`,`n`.`titulo` AS `titulo`,`n`.`subtitulo` AS `subtitulo`,`n`.`descripcion` AS `descripcion`,`n`.`redactor` AS `redactor`,`n`.`created_at` AS `created_at`,`n`.`updated_at` AS `updated_at`,`n`.`nvisitas` AS `nvisitas`,`n`.`image` AS `image` from (`tb_noticias` `n` join `tb_categorias` `c` on(`c`.`id` = `n`.`categoria`)) order by `n`.`id` desc */;
